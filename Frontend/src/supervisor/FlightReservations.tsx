...
import UpdateFlightRequest from "../components/UpdateFlightRequest";
...
export default function FlightReservation(props: any) {
    const [flightRequests, setFlightRequests] = useState([]);
    const [pageNumber, setPageNumber] = useState(0);
    const [openPopup, setOpen] = useState(false);
    const [currentRequest, setCurrentRequest] = useState<flightRequest>();

    const navigate = useNavigate();

    useEffect(() => {
        makeRequest(`/flights/requests`, 'POST', {
            pageSize: 5,
    ...
    const openPopupForRequest = (request: flightRequest) => {
    ...
    return (
        <div className="flex flex-col w-full p-4 border border-gray-400 rounded-lg">
            <Popup shouldopen={openPopup ? true : false} toggle={togglePopup} title="Request Status">
                <UpdateFlightRequest flightRequest={currentRequest} />
            </Popup>
            <div className="flex items-center justify-between p-4 font-semibold text-gray-600 bg-gray-300 rounded-lg">
                <p className="w-full text-center"> Flight </p>
                <p className="w-full text-center"> User </p>
            ...
            <div className="flex flex-col p-4 space-y-2 bg-white rounded-lg">
                {flightRequests != null && flightRequests.length < 1 && <p className="text-gray-400 italics"> No flight requests were found! </p>}
                {flightRequests.map((request: flightRequest) => 
                    <div className="flex items-center justify-between p-4 bg-gray-100 rounded-lg select-none">
                        <p className="w-full text-center"> {request.flight.id} </p>
                        <p className="w-full text-center"> {request.user.username} </p>
                        <p className="w-full text-center"> {request.status} </p>
                        <p onClick={() => openPopupForRequest(request)} className="w-full px-4 py-1 text-center text-white bg-blue-600 rounded-lg cursor-pointer hover:bg-blue-400"> Update Status </p>
                    </div>
                )}
            </div>
            <div className="my-2 border border-gray-400"></div>
