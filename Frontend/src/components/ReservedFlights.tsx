...
export default function ReservedFlights(props: any) {
    const navigate = useNavigate();
    const [flightRequests, setFlightRequests] = useState([]);
    const [pageNumber, setPageNumber] = useState(0);

    useEffect(() => {
        makeRequest(`/flights/requests/users/${props.userId}`, 'POST', {
            pageSize: 5,
    ...
    const showReason = (frequest : flightRequest) => {
        if(frequest.rejection_reason != null && frequest.rejection_reason != '') {
            Swal.fire(`Reason: ${frequest.rejection_reason}`);
        }
    }
            ...
            <div className="flex flex-col p-4 space-y-2 bg-white rounded-lg">
                ...
                {flightRequests.map((request: flightRequest) => 
                    <div className="flex items-center justify-between p-4 bg-gray-100 rounded-lg select-none">
                        ...
                        <p onClick={() => showReason(request)} className={`${request.status === 'Rejected' ? 'cursor-pointer text-red-600' : ''}`}> {request.status} </p>
                    </div>
                )}
            </div>
            <div className="my-2 border border-gray-400"></div>
